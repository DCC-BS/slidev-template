...
export class AnimationSequenceBuilder {
    ...
    quickStep(
        animations: Record<any, Record<string, any>>,
        options?: AnimationProps,
    ): this {
    ...
    moveStep(
        moves: Record<any, { x?: number; y?: number }>,
        options?: AnimationProps,
    ): this {
        return this.step((builder) => {
...
export function useEasyAnimation(
    options: {
    ...
    const createAnimationFromSequence = (sequence: {
        targets: any[];
        steps: TargetAnimation[][];
    }) => {
        ...
        const animationTargets = sequence.targets.map((target) => {
            const steps = sequence.steps.map((stepAnimations) => {
                ...
                const targetAnimation = stepAnimations.find(
...
export const AnimationPresets = {
    ...
    bounce: (target: any, options?: AnimationProps) => ({
        ...
        options: {
            ...options,
            easing: "bounceOut" as EasingPreset,
            duration: 300,
        },
    }),
