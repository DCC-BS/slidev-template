...
class GlobalAnimationManager {
  ...
  private startLoop() {
    ...
    const animate = (currentTime: number) => {
      ...
      const allUpdates: Array<{ target: any; updates: Record<string, number> }> = [];
...
class OptimizedAnimation {
  ...
  update(currentTime: number): Array<{ target: any; updates: Record<string, number> }> | null {
    if (!this.startTime) {
    ...
    for (const anim of this.animations) {
      ...
      for (let i = 0; i < anim.keys.length; i++) {
        targetUpdates[anim.keys[i]] = anim.startVals[i] + anim.diffs[i] * easedProgress;
      }
...
export function useOptimizedKonvaAnimation(
  ...
  const unwatchClicks = $slidev ? watch(() => $slidev.nav.clicks, (clicks) => {
    if (clicks === 0) {
      initializeTargets();
    } else {
      const targetStep = Math.min(clicks - 1, totalSteps.value - 1);
      animateToStep(targetStep);
    }
  }, { immediate: true }) : null;

  const unwatchPage = $slidev ? watch(() => $slidev.nav.currentPage, () => {
    stopAnimation();
    initializeTargets();
  }) : null;
